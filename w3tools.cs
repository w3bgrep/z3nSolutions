		...
		public static T nonce<T>(IZennoPosterProjectModel project, string chainRPC = "", string address = "", string proxy = "")
{
    ...
    return (T)Convert.ChangeType(transactionCount, typeof(T));
		...
		public static T native<T>(IZennoPosterProjectModel project, string chainRPC = "", string address = "", string proxy = "",bool log = false)
		{
			...
			project.SendInfoToLog($"[Leaf.xNet] Баланс нативной валюты для адреса {address}: {balanceNative}");

			if (typeof(T) == typeof(string))
				return (T)Convert.ChangeType(balanceNative.ToString("0.##################", CultureInfo.InvariantCulture), typeof(T));
	...
	public static class Time
	{
		...
		public static string cd(object input = null)
		{
		    ...
		    else if (input is decimal || input is int)  // Проверяем оба типа
		    {
		        // Конвертируем входное значение в decimal
	...
	public static class Onchain
	{
		...
		public static string HexToString(string hexValue, string convert = "")
		{
		    try
		    {
		        ...
		        // Конвертация в зависимости от параметра
		...
		public static T getNonce<T>(IZennoPosterProjectModel project, string chainRPC = "", string address = "", string proxy = "")
		{
			...
			if (chainRPC == "") chainRPC = project.Variables["blockchainRPC"].Value;
		    string jsonBodyGetNonce = $@"{{""jsonrpc"": ""2.0"",""method"": ""eth_getTransactionCount"",""params"": [""{address}"", ""latest""],""id"": 1}}";
		    string responseGetNonce = ZennoPoster.HttpPost(chainRPC, jsonBodyGetNonce, "application/json", proxy, "UTF-8", ZennoLab.InterfacesLibrary.Enums.Http.ResponceType.BodyOnly, 5000, "", "Mozilla/5.0", true, 5, null, "", false);
		
		    var match = System.Text.RegularExpressions.Regex.Match(responseGetNonce, @"""result""\s*:\s*""([^""]+)""");
		    string hexResultNonce = match.Success ? match.Groups[1].Value : "0x0";
		...
		public static T getNative<T>(IZennoPosterProjectModel project, string chainRPC = "", string address = "", string proxy = "")
		{
		    
			if (address == "") address = project.Variables["addressEvm"].Value;
			if (chainRPC == "") chainRPC = project.Variables["blockchainRPC"].Value;
			string jsonBody = $@"{{ ""jsonrpc"": ""2.0"", ""method"": ""eth_getBalance"", ""params"": [""{address}"", ""latest""], ""id"": 1 }}";
		    string responseBalance = ZennoPoster.HttpPost(chainRPC, jsonBody, "application/json", proxy, "UTF-8", ZennoLab.InterfacesLibrary.Enums.Http.ResponceType.BodyOnly, 5000, "", "Mozilla/5.0", true, 5, null, "", false);
		
		    var match = System.Text.RegularExpressions.Regex.Match(responseBalance, @"""result""\s*:\s*""([^""]+)""");
		    string hexResultBalance = match.Success ? match.Groups[1].Value.TrimStart('0', 'x') : "0";
		    BigInteger balanceWei = BigInteger.Parse("0" + hexResultBalance, NumberStyles.AllowHexSpecifier);
		    decimal balanceNative = (decimal)balanceWei / 1000000000000000000m;
		...
		public static T getGas<T>(IZennoPosterProjectModel project, string chainRPC = "", string proxy = "")
		{
		    if (chainRPC == "") chainRPC = project.Variables["blockchainRPC"].Value;
			string jsonBody = @"{""jsonrpc"":""2.0"",""method"":""eth_gasPrice"",""params"":[],""id"":1}";
		    string responseGas = ZennoPoster.HttpPost(chainRPC, jsonBody, "application/json", proxy, "UTF-8", ZennoLab.InterfacesLibrary.Enums.Http.ResponceType.BodyOnly, 5000, "", "Mozilla/5.0", true, 5, null, "", false);
		
		    var match = System.Text.RegularExpressions.Regex.Match(responseGas, @"""result""\s*:\s*""([^""]+)""");
		    string hexResultGas = match.Success ? match.Groups[1].Value.TrimStart('0', 'x') : "0";
		...
		public static T getERC20<T>(IZennoPosterProjectModel project, string tokenContract, string chainRPC = "", string address = "", string tokenDecimal = "18", string proxy = "")
		{
		    if (address == "") address = project.Variables["addressEvm"].Value;
		    if (chainRPC == "") chainRPC = project.Variables["blockchainRPC"].Value;
		    ...
		    string jsonBody = $@"{{ ""jsonrpc"": ""2.0"", ""method"": ""eth_call"", ""params"": [{{ ""to"": ""{tokenContract}"", ""data"": ""{data}"" }}, ""latest""], ""id"": 1 }}";
		    string response = ZennoPoster.HttpPost(chainRPC, jsonBody, "application/json", proxy, "UTF-8", ZennoLab.InterfacesLibrary.Enums.Http.ResponceType.BodyOnly, 5000, "", "Mozilla/5.0", true, 5, null, "", false);
		
		    var match = System.Text.RegularExpressions.Regex.Match(response, @"""result""\s*:\s*""([^""]+)""");
		    string hexResult = match.Success ? match.Groups[1].Value.TrimStart('0', 'x') : "0";
		...
		public static T getChainId<T>(IZennoPosterProjectModel project, string chainRPC = "", string proxy = "")
		{
		    if (chainRPC == "") chainRPC = project.Variables["blockchainRPC"].Value;
		    
		    string jsonBodyGetChainId = @"{""jsonrpc"": ""2.0"",""method"": ""eth_chainId"",""params"": [],""id"": 1}";
		    string responseGetChainId = ZennoPoster.HttpPost(chainRPC, jsonBodyGetChainId, "application/json", proxy, "UTF-8", ZennoLab.InterfacesLibrary.Enums.Http.ResponceType.BodyOnly, 5000, "", "Mozilla/5.0", true, 5, null, "", false);
		
		    var match = System.Text.RegularExpressions.Regex.Match(responseGetChainId, @"""result""\s*:\s*""([^""]+)""");
		    string hexResultChainId = match.Success ? match.Groups[1].Value : "0x0";
		...
		public static T getERC1155<T>(IZennoPosterProjectModel project, string tokenContract, string tokenId, string chainRPC = "", string address = "", string proxy = "")
		{
			// Устанавливаем значения по умолчанию из переменных проекта, если не указаны
			if (address == "") address = project.Variables["addressEvm"].Value;
			if (chainRPC == "") chainRPC = project.Variables["blockchainRPC"].Value;

			// Формируем данные для вызова balanceOf(address account, uint256 id)
			string functionSelector = "0x00fdd58e"; // keccak256("balanceOf(address,uint256)")[:4]
			string paddedAddress = address.Replace("0x", "").ToLower().PadLeft(64, '0'); // Адрес без 0x, дополненный до 32 байт
			string paddedTokenId = BigInteger.Parse(tokenId).ToString("x").PadLeft(64, '0'); // tokenId в hex, дополненный до 32 байт
			string data = functionSelector + paddedAddress + paddedTokenId;

			// Формируем JSON-RPC запрос для eth_call
			string jsonBody = $@"{{
				""jsonrpc"": ""2.0"",
				""method"": ""eth_call"",
				""params"": [{{
			...
			// Отправляем запрос через HttpPost
			string response = ZennoPoster.HttpPost(chainRPC, jsonBody, "application/json", proxy, "UTF-8", ZennoLab.InterfacesLibrary.Enums.Http.ResponceType.BodyOnly, 5000, "", "Mozilla/5.0", true, 5, null, "", false);
			...
			project.SendInfoToLog($"Баланс токена ERC-1155 (tokenId: {tokenId}) для адреса {address}: {balance}");

			// Возвращаем результат в нужном типе
			if (typeof(T) == typeof(string))
				return (T)Convert.ChangeType(balance.ToString(), typeof(T));
		...
		public static T getERC721<T>(IZennoPosterProjectModel project, string tokenContract, string chainRPC = "", string address = "", string proxy = "")
		{
			// Устанавливаем значения по умолчанию из переменных проекта, если не указаны
			if (address == "") address = project.Variables["addressEvm"].Value;
			if (chainRPC == "") chainRPC = project.Variables["blockchainRPC"].Value;

			// Формируем данные для вызова balanceOf(address owner)
			string functionSelector = "0x70a08231"; // keccak256("balanceOf(address)")[:4]
			string paddedAddress = address.Replace("0x", "").ToLower().PadLeft(64, '0'); // Адрес без 0x, дополненный до 32 байт
			string data = functionSelector + paddedAddress;
		...
		public static T GetInitiaBalances<T>(IZennoPosterProjectModel project, string chain = "initiation-2", string token = "", string address = "")
		{
		...
		//GetTx
		public static string getTransactionReceipt(IZennoPosterProjectModel project, string chainRPC, string txHash, string proxy = "")
		{
		    string jsonBody = $@"{{""jsonrpc"":""2.0"",""method"":""eth_getTransactionReceipt"",""params"":[""{txHash}""],""id"":1}}";
		    
		    string responseReceipt = ZennoPoster.HttpPost(chainRPC, jsonBody, "application/json", "", "UTF-8", ZennoLab.InterfacesLibrary.Enums.Http.ResponceType.BodyOnly, 5000, "", "Mozilla/5.0", true, 5, null, "", false);
		
		    // Проверяем наличие результата
		    if (string.IsNullOrWhiteSpace(responseReceipt) || responseReceipt.Contains("\"result\":null"))
		        return null;
		
		    return responseReceipt;
		}
		public static string getTransactionRaw(IZennoPosterProjectModel project, string chainRPC, string txHash, string proxy = "")
		{
		    string jsonBody = $@"{{""jsonrpc"":""2.0"",""method"":""eth_getTransactionByHash"",""params"":[""{txHash}""],""id"":1}}";
		    
		    string responseTx = ZennoPoster.HttpPost(chainRPC, jsonBody, "application/json", "", "UTF-8", ZennoLab.InterfacesLibrary.Enums.Http.ResponceType.BodyOnly, 5000, "", "Mozilla/5.0", true, 5, null, "", false);
		
		    // Проверяем наличие результата
		    if (string.IsNullOrWhiteSpace(responseTx) || responseTx.Contains("\"result\":null"))
		        return null;
		...
		public static string getTx(IZennoPosterProjectModel project, string chainRPC = "", int deadline = 60)
		{
			string hash = project.Variables["blockchainHash"].Value;
			if (chainRPC == "") chainRPC = project.Variables["blockchainRPC"].Value;
			string jsonRecept = $@"{{""jsonrpc"":""2.0"",""method"":""eth_getTransactionReceipt"",""params"":[""{hash}""],""id"":1}}";
			string jsonRaw = $@"{{""jsonrpc"":""2.0"",""method"":""eth_getTransactionByHash"",""params"":[""{hash}""],""id"":1}}";
			string response = "";
			...
			while (true)
			{
				...
				var logString = "";
				try {
					response = Http.W3Post(project, chainRPC, jsonRecept);
					if (!string.IsNullOrEmpty(response)) {
						project.Json.FromString(response);
					}
				} catch {
				...
				try
				{
					string gasUsed = HexToString(project.Json.result.gasUsed, "gwei");
					string gasPrice = HexToString(project.Json.result.effectiveGasPrice, "gwei");
					string status = HexToString(project.Json.result.status);
				...
				catch { project.Variables["txStatus"].Value = "noStatus"; }
				
				Thread.Sleep(1000);
				
				try {
					response = Http.W3Post(project, chainRPC, jsonRaw);
					if (!string.IsNullOrEmpty(response)) {
						project.Json.FromString(response);
					}
				} catch {
				...
				try
				{
					string gas = HexToString(project.Json.result.maxFeePerGas, "gwei");
					string gasPrice = HexToString(project.Json.result.gasPrice, "gwei");
					string nonce = HexToString(project.Json.result.nonce);
					string value = HexToString(project.Json.result.value, "eth");
					project.Variables["txStatus"].Value = "PENDING";
		...
		public static string SendTransaction(string chainRpc, string contractAddress, string encodedData, decimal value, string walletKey, int speedup = 1)
		{
		    try 
		    {
		        ...
		        BigInteger adjustedGasPrice = gasPrice + (gasPrice / 10 * speedup);
		        ...
		        var gasLimit = gasEstimate.Value + (gasEstimate.Value / 10);
		...
		
		{
			...
			if ( chainRPC == "")
			{
				...
				{
				    chainRPC = RPC.Trim();
					var native = getNative<decimal>(project, RPC, accountAddress);
			...
			{
				var native = getNative<decimal>(project, chainRPC, accountAddress);
			...
			string result = getTx(project,chainRPC);
			return result;
		}
	...
	public static class CEX
	{
        ...
        public static string BinanceGetUserAsset(IZennoPosterProjectModel project, string coin = "")
		{
		...
		public static string BinanceWithdraw(IZennoPosterProjectModel project, string amount, string network, string coin = "ETH", string address = "")
		{
			string[] keys = Db.BinanceApiKeys(project).Split(';');
        ...
        public static string BinanceGetWithdrawHistory(IZennoPosterProjectModel project, string searchId = "")
        {
			string[] keys = Db.BinanceApiKeys(project).Split(';');
	...
	public static class Wallets
	{
        ...
        #region Metamask
        public static void MMConfirm(this Instance instance)
		{
			int i = 0;
			DateTime deadline = DateTime.Now.AddSeconds(60);
	    ...
	    public static void MMConfirm2(this Instance instance,IZennoPosterProjectModel project, bool log = false )
		{
			...
			while (!instance.ActiveTab.URL.Contains("nkbihfbeogaeaoehlefnkodbefgpgknn"))
			{
				...
				// Проверка: если прошло более 6 попыток (6 секунд), выбрасываем исключение
				if (attemptCount > 5)
				{
			...
			// Ожидание завершения взаимодействия с MetaMask (клик по кнопке)
			if (log)project.SendInfoToLog("Starting button click loop on MetaMask page...");
			while (instance.ActiveTab.URL.Contains("nkbihfbeogaeaoehlefnkodbefgpgknn"))
			{
				// Проверка дедлайна (60 секунд с начала)
        ...
        #region Keplr
		public static void KeplrApprove(this Instance instance,IZennoPosterProjectModel project)
		{
		...
		public static class Tools
	{
		...
		public static string MultiplyEmail(string email, int count = 1)
		{
		    ...
		    // Проверяем, возможно ли создать требуемое количество уникальных вариантов
		    int maxPossibleDots = username.Length - 2; // -2 потому что исключаем первую и последнюю позиции
		    ...
		    {
		        var position = random.Next(1, username.Length - 1); // Начинаем с 1 и заканчиваем length-2
