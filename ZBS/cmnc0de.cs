    ...
    public class Stargate2
    {
        ...
        public Dictionary<string, decimal> DicToken(string[] chainsToUse = null, bool log = false) //usde hardcoded
        {
            ...
            foreach (string chain in chainsToUse)
            {
                decimal bal;
                if (!chain.Contains("solana")) 
                    bal = _w3b.BalERC20<decimal>("0x5d3a1Ff2b6BAb83b63cd9AD0787074081a52ef34", _w3b.Rpc(chain));                 
                else 
                    bal = _w3b.TokenSPL<decimal>("DEkqHyPN7GMRJ5cArtQFAWefqbZb33Hyf6s5iCwjEonT");
